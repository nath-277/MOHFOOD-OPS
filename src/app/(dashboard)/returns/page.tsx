...
export default function DamagesPage() {
  const [activeTab, setActiveTab] = useState<"ALL" | "FLOOR" | "WAREHOUSE">("ALL");
  ...
  // Sync hash with active tab (#warehouse vs #floor vs all)
  useEffect(() => {
    const handleHash = () => {
      const hash = window.location.hash.replace("#", "").toLowerCase();
      if (hash === "warehouse") {
        setActiveTab("WAREHOUSE");
      } else if (hash === "floor" || hash === "scrap") {
        setActiveTab("FLOOR");
      } else {
        setActiveTab("ALL");
      }
    ...
    const handleCustomTab = (e: Event) => {
      ...
      if (custom.detail === "warehouse") {
        setActiveTab("WAREHOUSE");
      } else if (custom.detail === "floor" || custom.detail === "scrap") {
        setActiveTab("FLOOR");
      } else {
        setActiveTab("ALL");
      }
  ...
  const switchTab = (tab: "ALL" | "FLOOR" | "WAREHOUSE") => {
    setActiveTab(tab);
    setSearchQuery("");
    if (typeof window !== "undefined") {
      window.location.hash = tab === "ALL" ? "" : tab.toLowerCase();
    }
  };
  ...
  const loadData = useCallback(async () => {
    try {
      setRefreshing(true);
      const [returnsAuditRes, damagesRes, itemsRes] = await Promise.all([
  ...
  const totalLossIncidents = useMemo(() => {
    return unifiedScrapList.filter((i) => i.categoryType !== "EXCESS").length;
  }, [unifiedScrapList]);

  const warehouseDamageCount = useMemo(() => {
    return unifiedScrapList.filter((i) => i.source === "WAREHOUSE").length;
  }, [unifiedScrapList]);

  const floorScrapCount = useMemo(() => {
    return unifiedScrapList.filter((i) => i.source === "FLOOR" && i.categoryType !== "EXCESS").length;
  }, [unifiedScrapList]);

  // Filtered Plant Floor Scrap & Damages
  const filteredScrap = useMemo(() => {
    let list = unifiedScrapList;

    if (activeTab === "FLOOR") {
      list = list.filter((i) => i.source === "FLOOR");
    } else if (activeTab === "WAREHOUSE") {
      list = list.filter((i) => i.source === "WAREHOUSE");
    }
    ...
    return list;
  }, [unifiedScrapList, activeTab, scrapCategoryFilter, scrapRootCauseFilter, searchQuery]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight mt-1">
            Damages & Plant Floor Scrap
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Unified governance for plant floor scrap, warehouse material damages, recipe rejects, and spoilage.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5">
        <div className="p-3 sm:p-4 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] sm:text-[11px] font-medium text-slate-500 mt-0.5 truncate">
              {totalLossIncidents} write-off events
            </div>
          </div>
        ...
        <div className="p-3 sm:p-4 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-base sm:text-2xl font-bold text-slate-900 mt-0.5 sm:mt-1 font-mono truncate">
              {floorScrapCount} <span className="text-xs font-normal text-slate-500">Events</span>
            </div>
            <div className="text-[10px] sm:text-[11px] font-medium text-slate-500 mt-0.5 truncate">
              Production batch rejects
            </div>
          </div>
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center shrink-0 ml-2">
        ...
        <div className="p-3 sm:p-4 rounded-xl bg-white border border-slate-200 shadow-xs flex items-center justify-between">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-base sm:text-2xl font-bold text-slate-900 mt-0.5 sm:mt-1 font-mono truncate">
              {warehouseDamageCount} <span className="text-xs font-normal text-slate-500">Events</span>
            </div>
            <div className="text-[10px] sm:text-[11px] font-medium text-slate-500 mt-0.5 truncate">
              Storage spoilage & breakages
            </div>
          </div>
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center shrink-0 ml-2">
      ...
      {/* Main Tabs Navigation: All vs Plant Floor vs Warehouse */}
      <div className="flex border-b border-slate-200 bg-white rounded-t-2xl px-3 sm:px-6 pt-3 gap-2 sm:gap-4 overflow-x-auto no-scrollbar shadow-2xs">
        ...
        >
          <Layers className={`w-4 h-4 ${activeTab === "ALL" ? "text-[#CF0458]" : "text-slate-400"}`} />
          <span>All Damages & Scrap</span>
          <span
        ...
        >
          <AlertOctagon className={`w-4 h-4 ${activeTab === "FLOOR" ? "text-[#CF0458]" : "text-slate-400"}`} />
        ...
        >
          <Boxes className={`w-4 h-4 ${activeTab === "WAREHOUSE" ? "text-[#CF0458]" : "text-slate-400"}`} />
